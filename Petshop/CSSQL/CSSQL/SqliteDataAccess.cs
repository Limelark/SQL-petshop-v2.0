...
namespace CSSQL
{
    public class SqliteDataAccess
    {
        ...
        public static List<ProductModel> SortProduct(string symbol, int number)
        {
            using (IDbConnection cnn = new SQLiteConnection(LoadConnectionString()))
            {
                List<ProductModel> output = cnn.Query<ProductModel>("SELECT * FROM Products WHERE Price " + symbol + " " + number, new DynamicParameters()).ToList();
                return output;
            }
        }
